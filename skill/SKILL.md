...
可参考：`references/template-iteration-lessons-2026-05-06.md`
...
## GitHub 交付约束：若用户要“仓上只有一个 commit”

若用户明确要求 GitHub 上只保留一个 commit：

1. 先停止继续推送增量提交
2. 先把本地版本全部改完
3. 明确告诉用户：先不提交，最后一次覆盖远端
4. 最后再在本地压平历史，并 `git push --force`
5. 用 `git rev-list --count HEAD` 与远端 SHA 实际校验

不要在用户说要单提交历史之后，仍继续边改边 push 多个 commit。

这是此类“模板仓整理 / 展示仓收束”任务的硬边界。

- 若需查看本轮从 ref-page 模式迁到 component-library 的收束，可看 `references/component-library-migration-2026-05-06.md`
...
当一轮模板迭代结束后，`references/` 不应堆满过程笔记。

优先做法：

- 把可复用结论收成一份总文档
- 删除一次性发现、重复记录、已被吸收的碎片文档
...
- `references/` 是否已收束为少量长期有效文档
- 若用户要求单提交历史，远端是否确为 1 commit
- GitHub 远端是否已同步
